...
//			coef_single[j] = 0.5*(ap.f_ff1 + coef_single_prev[j]);
		...
		noise.add = (uint16_t)(sin(((float32_t)i)/100000)*30000 + 30000);
		noise.mul = (uint16_t)(sin(((float32_t)i)/100405)*30000 + 30000);
		noise.mod = (uint16_t)(sin(((float32_t)i)/105003)*30000 + 30000);
...
//			vector_a[l] += ((ADC3ConvertedVoltage[0]+0.1)*vector_b[l]*sin(((float32_t)i/100.0)*3.14159*2));
			vector_a[l] += ((ADC3ConvertedVoltage[0]+0.05)*vector_d[i%DELSIZE]);
			if(vector_a[l]>1.0) vector_a[l] = 0.96;
			else if(vector_a[l]<-1.0) vector_a[l] = -0.96;
...
//		outvalue = vector_a[i];
		...
		{
			if(vector_b[l]>1.0) vector_b[l] = 0.96;
			else if(vector_b[l]<-1.0) vector_b[l] = -0.96;
