...
__IO uint32_t ADC3ConvertedVoltage[4] = {0, 0, 0, 0};
__IO uint16_t vector_a[4] = {0, 0, 0, 0};
__IO uint16_t vector_b[4] = {0, 0, 0, 0};
__IO uint16_t vector_c[4] = {0, 0, 0, 0};
__IO uint16_t vector_d[4] = {0, 0, 0, 0};
__IO int sig = 0;
...
void Blink(__IO uint32_t nCount, uint32_t led)
{
    GPIOD->BSRRL = led; // set PD12 thru PD15
    Delay(nCount);		 // wait a short period of time
    GPIOD->BSRRH = led; // reset PD12 thru PD15
    Delay(nCount);
}
...
//say hello:
    ...
    while (1)
    {
    	...
    	if((counter%16)<4)
    	{
    		DAC1ConvertedValue = vector_a[(i-4)%4] - vector_a[i];
    	} else if((counter%16)<8)
    	{
    		DAC1ConvertedValue = vector_b[(i-4)%4] - vector_b[i];
    	} else if((counter%16)<12)
    	{
    		DAC1ConvertedValue = vector_c[(i-4)%4] - vector_c[i];
    	} else if((counter%16)<16)
    	{
    		DAC1ConvertedValue = vector_d[(i-4)%4] - vector_d[i];
    	};
    	*/
		...
		outvalue%=4096;
    	DAC1ConvertedValue = outvalue;
		//DAC1ConvertedValue = ADC3ConvertedValue[0];
        ...
        {
        	ADC3ConvertedVoltage[j] = (uint32_t)ADC3ConvertedValue[j]*100L;
        };

        if((counter%ADC3ConvertedVoltage[0])<(ADC3ConvertedVoltage[0]/2))
        {
        	sig = 1;
        	GPIOD->BSRRH = 0xF000;
        } else if((counter%ADC3ConvertedVoltage[0])>(ADC3ConvertedVoltage[0]/2))
        {
        	sig = 0;
        	GPIOD->BSRRL = 0xF000;
        };
		...
		//Blink((uint32_t)ADC3ConvertedValue[3]*1000L, 0x8000);
		//Blink((uint32_t)ADC3ConvertedValue[2]*1000L, 0x1000);
        //Blink((uint32_t)ADC3ConvertedValue[1]*1000L, 0x2000);
        //Blink((uint32_t)ADC3ConvertedValue[0]*1000L, 0x4000);
