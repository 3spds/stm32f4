...
//init:
    ...
    //init in/out arrays (for filter)
    for(j=0; j<2; j++)
        out[j] = in[j] = 0;
    //additional setup for filter

    w0 = 2*pi*f0/inv_samp_rate;
    alpha = sin(w0)/(2*q);
...
//say hello:
    ...
    while (1)
    {
        ...
        in[(int)counter%2] = ADC3ConvertedValue;

        out[(int)counter%2] = (b0/a0) * in[(int)counter%2] +
            (b1/a0)*in[((int)counter-1)%2] + (b2/a0)*in[((int)counter-2)%2]
            - (a1/a0)*out[((int)counter-1)%2] + (a2/a0)*out[((int)counter-2)%2];
        //DAC1ConvertedValue = (uint16_t)sin((float)counter*inv_samp_rate*2*pi);
        //DAC1ConvertedValue = (uint16_t)out[(int)counter%2];
        DAC1ConvertedValue = ADC3ConvertedValue * (uint16_t)sin(counter*inv_samp_rate*2*pi);
        Pulse();
        counter ++;
    }
}
