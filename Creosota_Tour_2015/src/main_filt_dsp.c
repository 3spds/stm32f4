...
//			vector_a[l] *= 0.1;
//			vector_a[l] = 0.2*sin(((float32_t)i/100.0)*3.14159*2);
//			if(!(i%RATE)) vector_a[l] = 1;
//			else vector_a[l] = 0;

			melody.add = gamut[noise.out] / 200;
			melody.slew = 0.999 + (0.001 * (1-ADC3ConvertedVoltage[1]));
			harmony.add = gamut[noise3.out] / 400;
			harmony.slew = 0.999 + (0.001 * (1-ADC3ConvertedVoltage[1]));
			saw.add = gamut[noise.out + 2 % gsize] / 800;
			saw.slew = 0.999 + (0.001 * (1-ADC3ConvertedVoltage[1]));
			drone.add = gamut[noise2.out] / 25;
			dronelo.add = gamut[noise2.out + 7 % gsize] / 800;
			vector_a[l] = (0.3*run_phasor(&saw))*((1-(ADC3ConvertedVoltage[0]*0.1)));
			vector_a[l] += (0.3*run_phasor(&melody))*((1-(ADC3ConvertedVoltage[0]*0.1)));
			vector_a[l] += (0.2*run_phasor(&harmony))*((1-(ADC3ConvertedVoltage[0]*0.1)));
			vector_a[l] += (0.1*run_phasor(&drone))*((1-(ADC3ConvertedVoltage[0]*0.1)));
			vector_a[l] += (0.2*run_phasor(&dronelo))*((1-(ADC3ConvertedVoltage[0]*0.1)));
...
//			DAC2ConvertedValue[l] = (uint16_t)(((vector_b[l])+1)*2047);
		}
...
void init_GPIOB(void)
{
	...
	//GPIOA init (outputs)
    GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1;		  // we want to configure PA04
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AN; 	  // we want it to be analog
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_100MHz;//this sets the GPIO modules clock speed
    GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;   // this sets the pin type to push / pull (as opposed to open drain)
    GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_NOPULL;
